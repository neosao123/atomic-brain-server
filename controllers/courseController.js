...
module.exports = {
  ...
  list: async (req, res) => {
    try {
      const data = await CourseModel.find({ isDelete: false })
        .sort({ createdAt: -1 })
        .exec();
      ...
      data.forEach((element) => {
        if (
          element.image !== "" &&
          fs.existsSync("uploads/courses/" + element.image)
        ) {
          element.image = image_url + element.image;
        } else {
          element.image = "";
        }
      });
      return res
        .status(200)
        .send({ err: 200, message: "Data Found", data: data });
    } catch (error) {
      res.status(500).send({ err: 300, message: error.toString() });
    }
  },
  ...
  edit: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ err: 200, message: "Data found", data: course });
    } catch (error) {
  ...
  update: async (req, res) => {
    try {
      ...
      if (req.file !== undefined) {
        if (imageName != "") {
          deleteFile(
            path.resolve(__dirname, "../uploads/courses/" + imageName)
          );
        }
      }
      const { title, shortDescription, description, type, price } = req.body;
