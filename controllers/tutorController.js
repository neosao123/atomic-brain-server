...
module.exports = {
	...
	UpdateProfile: async (req, res) => {
		try {
			...
			if (user.length > 0) {
				return res.status(200).send({ err: 300, message: "This email-id has already been used. Please try with another email-id" });
			}
			...
			return res.status(200).send({ err: 300, message: "No changes where found to update.", data: updateResult });
		} catch (ex) {
